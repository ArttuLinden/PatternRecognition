...
all_scores = []
for l,f,c in [(l,f,c) for l in pca_n for f in feats for c in zip(clfs,clf_names)]:

    scores = testClassifier(data,c[0],'vel_acc',f,n_splits,False,l)
    all_scores.append([c[1],l,f,scores])
    
    print("{:.5f} accuracy for pca={}, {}, {}".format(np.mean(scores),l,f,c[1]))
